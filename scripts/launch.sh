...
burn_fee=2.5
source_miner="src/comchat/miner/anthropic.py"
source_validator="src/comchat/validator/text_validator.py"

# Install ComChat
install_comchat() {
    # Downloading comchat
    echo "Installing ComChat"
    git clone https://github.com/agicommies/comchat.git
    cd comchat || exit
...
create_setup() {
    ...
    echo "The instillation will only work on Linux. If you are on Windows, please refer to the ComChat readme for instructions."
    echo "https://github.com/agicommies/comchat/blob/main/README.md"
    # shellcheck disable=SC2162
    read -p "Install ComChat (y/n): " install_comchat
    if [ "$install_comchat" = "y" ]; then
        install_comchat
    fi
    ...
    args: `module serve --ip "${process.env.MODULE_IP}" --port "${process.env.MODULE_PORT}" --subnets-whitelist "${process.env.MODULE_NETUID}" "comchat.miner.${process.env.MODULE_PATH}" "${process.env.MODULE_KEYNAME}"`,
            watch: true,
            env: {
                MODULE_ENV: 'development',
...
configure_launch() {
    ...
    if [ "$is_miner" = "true" ]; then
        if [ ! -f "src/comchat/miner/$filename.py" ]; then
            # Copy the source miner file to the destination
            destination_file="src/comchat/miner/$filename.py"
            ...
            echo "Miner module created at src/comchat/miner/$filename.py"
        fi
    fi

    # Create the validator module if it doesn't exist
    if [ "$is_validator" = "true" ]; then
        if [ ! -f "src/comchat/validator/$filename.py" ]; then
            # Copy the source validator file to the destination
            destination_file="src/comchat/validator/$filename.py"
            cp "$source_validator" "$destination_file"
            # Replace the class name in the destination file
            sed -i "s/TextValidator/$classname/g" "$destination_file"
            echo "Validator module created at src/comchat/validator/$filename.py"
        fi
    fi
...
serve_miner() {
    echo "Serving Miner"
    pm2 start "comx module serve comchat.miner.$module_path ${key_name} --ip  $ip_address --port $port --subnets-whitelist $netuid" --name "$module_path"
    echo "Miner served."
}
...
serve_validator() {
    ...
    pm2 start "python -m comchat.cli $module_path"
